"""
DEPRECATED: processor_watchdog.py — Legacy file-system watcher + forensic analyser.

THIS FILE IS DEPRECATED — DO NOT USE IN PRODUCTION
========================================================

This version attempted to:
  1. Load and MERGE all 6 DataFrames on "Ora" column
  2. Apply 6 forensic rules to every inverter × timestep
  3. Compute per-inverter health flags
  4. Write JSON snapshots

ISSUES IDENTIFIED (2026-04-02):
  - Memory exhaustion: Full left-joins on "Ora" with different-sized DataFrames
    caused allocation failure (attempted 2.46 GiB) because:
    • Potenza_AC: 23,040 rows
    • Temperatura, Irraggiamento: 20,160 rows
    • Corrente_DC: 11,520 rows
    • PR: 720 rows (long format, not time-series)
    Full cross-merge created 73 × 4.5M cells → OOM

  - Data structure mismatch: Merged code couldn't find inverter columns
    due to column name changes (e.g., "Potenza AC (INV TX1-01) [W]")

  - Time-based merging: Matching on "Ora" (decimal hours) across files
    with different extraction frequencies/timestamps was unreliable

REPLACEMENT: processor_watchdog_final.py
==========================================

Use processor_watchdog_final.py instead. It:
  ✅ Avoids full merges (memory-safe)
  ✅ Uses Potenza_AC as master, lookups only
  ✅ Finds latest valid (non-NaN) data automatically
  ✅ Time-aware thresholds (DC current varies by hour of day)
  ✅ Tested and working with real data

Migration:
  - run_monitor.py updated to use processor_watchdog_final.py
  - This file kept for reference/audit only
  - Do not run: python processor_watchdog_deprecated.py

See ANALYSIS_FIX_SUMMARY.md for complete details.

---

ORIGINAL DOCSTRING (for reference):
File-system watcher + forensic analyser.
