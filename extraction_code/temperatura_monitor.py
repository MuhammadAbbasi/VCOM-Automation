import logging
import time
import pandas as pd
from .base_monitor import toggle_minute_values, refresh_chart, click_dati_tab, extract_infotab_table_js, dismiss_popup

logger = logging.getLogger(__name__)


def extract_temperatura(page) -> pd.DataFrame:
    """Extract Temperatura data from the VCOM Evaluation dashboard."""
    logger.info("Clicking 'Temperatura' tab...")
    page.locator('text="Temperatura"').first.click()
    time.sleep(2)
    dismiss_popup(page)

    logger.info("Toggling minute values for Temperatura...")
    toggle_minute_values(page, "Temperatura")
    refresh_chart(page)

    logger.info("Switching to Dati tab for Temperatura...")
    click_dati_tab(page, extra_wait=1)

    logger.info("Extracting Temperatura table via JS...")
    return extract_infotab_table_js(page, "Temperatura")
