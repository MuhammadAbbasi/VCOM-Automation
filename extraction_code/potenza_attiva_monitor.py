...
def extract_potenza_attiva(page) -> pd.DataFrame:
    ...
    try:
        # Check if Regolazione della potenza attiva or Potenza attiva is already visible
        target = page.locator('text=/^\\s*(Regolazione della potenza attiva|Potenza attiva)\\s*$/i').first
        if not target.is_visible():
            logger.info("Clicking 'Power control' parent menu...")
            page.locator('text=/^\\s*Power control\\s*$/i').first.click(force=True)
            time.sleep(1)
        
        logger.info("Clicking 'Regolazione della potenza attiva' sub-menu...")
        ...
        page.locator('text=/^\\s*(Regolazione della potenza attiva|Potenza attiva)\\s*$/i').first.click(force=True)
